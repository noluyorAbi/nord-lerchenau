...
function voiceFor(sb, scene) {
  ...
  return {
    ...
    instructions:
      (scene && scene.voiceInstructions) ||
      meta.voiceInstructions ||
...
function loadStoryboard(p) {
  ...
  if (text === null)
    return {
      sb: null,
      errors: [`${p.storyboard} does not exist. Stage 1 (plan) writes it.`],
    };
  let sb;
  try {
    sb = JSON.parse(text);
    ...
    return {
      sb: null,
      errors: [`storyboard.json is not valid JSON: ${e.message}`],
    };
  }
  if (!sb.meta || typeof sb.meta !== "object")
    errors.push("meta object missing");
  if (!Array.isArray(sb.scenes) || !sb.scenes.length)
    errors.push("scenes array missing or empty");
  const seen = new Set();
  for (const s of sb.scenes || []) {
    const tag = s.id || "<no id>";
    if (!s.id || !/^[\w-]+$/.test(s.id))
      errors.push(`scene ${tag}: id must be filename-safe ([A-Za-z0-9_-])`);
    if (seen.has(s.id)) errors.push(`scene ${tag}: duplicate id`);
    seen.add(s.id);
    if (!KINDS.includes(s.kind))
      errors.push(`scene ${tag}: kind must be one of ${KINDS.join(", ")}`);
    if (!s.narration && s.kind !== "title")
      errors.push(
        `scene ${tag}: narration missing (only title scenes may stay silent)`,
      );
    if (s.kind === "browser" && !s.footage)
      errors.push(
        `scene ${tag}: browser scene needs footage (e.g. "footage/${tag}.webm")`,
      );
    if (s.kind === "cli" && !s.ansi)
      errors.push(
        `scene ${tag}: cli scene needs ansi (e.g. "footage/${tag}.ansi")`,
      );
    if (s.kind === "still" && !s.image)
      errors.push(
        `scene ${tag}: still scene needs image (e.g. "footage/${tag}.png")`,
      );
    if (s.variants !== undefined) {
      if (
        !Array.isArray(s.variants) ||
        !s.variants.length ||
        s.variants.some((v) => !["short", "long"].includes(v))
      )
        errors.push(
          `scene ${tag}: variants must be a non-empty array of "short" / "long" (omit for both)`,
        );
    }
    if (s.voice && !VOICES.includes(s.voice))
      errors.push(
        `scene ${tag}: unknown voice "${s.voice}" (known: ${VOICES.join(", ")})`,
      );
  }
  if (sb.meta && sb.meta.voice && !VOICES.includes(sb.meta.voice))
    errors.push(
      `meta.voice "${sb.meta.voice}" unknown (known: ${VOICES.join(", ")})`,
    );
  return { sb, errors };
}
...
function ttsEstimate(textChars, audioSeconds) {
  ...
  const usd =
    (textTokens / 1e6) * TTS_USD_PER_1M_TEXT_TOKENS +
    (audioSeconds / 60) * TTS_USD_PER_AUDIO_MINUTE;
...
function appendCost(p, entry) {
  ...
  try {
    fs.appendFileSync(path.join(p.video, "costs.jsonl"), line + "\n");
  } catch (_) {
