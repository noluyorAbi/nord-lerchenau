...
function compile(sb, narration, p, errors, variant) {
  ...
  const hold =
    (sb.meta && sb.meta.holdSeconds) != null ? sb.meta.holdSeconds : 0.6;
  const scenes = [];
  let cursor = 0;
  ...
  for (const s of sb.scenes) {
    ...
    if (s.narration && !n) {
      errors.push(
        `scene ${s.id}: narrated but not in narration.json (run narrate.js)`,
      );
      continue;
    }
    ...
    const durS = Math.max(
      MIN_SCENE_S,
      s.narration ? audioDur + hold : TITLE_SILENT_S,
    );
    ...
    const scene = {
      ...
      words: n
        ? n.words.map((w) => ({
            w: w.w,
            startFrame: Math.round(w.s * fps),
            endFrame: Math.round(w.e * fps),
          }))
        : [],
    };
    if (s.kind === "title") {
      scene.headline = s.headline || (sb.meta && sb.meta.title) || "";
      scene.sub = s.sub || "";
    }
    if (s.kind === "browser") {
      if (!L.exists(assetPath(p, s.footage)))
        errors.push(
          `scene ${s.id}: footage missing: video/public/${s.footage} (stage 2)`,
        );
      scene.footage = s.footage;
    }
    if (s.kind === "still") {
      if (!L.exists(assetPath(p, s.image)))
        errors.push(
          `scene ${s.id}: image missing: video/public/${s.image} (stage 2)`,
        );
      scene.image = s.image;
    }
    if (s.kind === "cli") {
      ...
      if (raw === null) {
        errors.push(
          `scene ${s.id}: ansi capture missing: video/public/${s.ansi} (stage 2)`,
        );
      } else {
...
function renderScenes(content) {
  const scenes = content.scenes.map((s) => {
    ...
    if (s.kind === "title") {
      extra.push(
        `    headline: ${JSON.stringify(s.headline)}, sub: ${JSON.stringify(s.sub)},`,
      );
    }
    if (s.kind === "browser")
      extra.push(`    footage: ${JSON.stringify(s.footage)},`);
    if (s.kind === "still")
      extra.push(`    image: ${JSON.stringify(s.image)},`);
    if (s.kind === "cli")
      extra.push(
...
function renderTs(short, long) {
  ...
  const usesAnsi = [...short.scenes, ...long.scenes].some(
    (s) => s.kind === "cli",
  );
...
function installTemplates(p, forceUpdate, notes) {
  ...
  for (const f of ["Vertical.tsx", "Wide.tsx", "demovideo-types.ts"]) {
    ...
    notes.push(
      `${L.exists(dst) && forceUpdate ? "updated" : "installed"} video/src/${f}`,
    );
  }
  ...
  if (pkgText) {
    ...
    const SCRIPTS = {
      "render:vertical":
        "remotion render src/index.ts Vertical out/vertical.mp4 --codec=h264 --crf=20 --log=error --timeout=120000",
      "render:wide":
        "remotion render src/index.ts Wide out/wide.mp4 --codec=h264 --crf=20 --log=error --timeout=120000",
      "render:wide-long":
        "remotion render src/index.ts WideLong out/wide-long.mp4 --codec=h264 --crf=20 --log=error --timeout=120000",
    };
    let added = [];
...
function main() {
  ...
  if (!L.exists(p.video)) {
    console.error(
      "video/ workspace missing. Run /launch stage v2 first (see SKILL.md).",
    );
    return 1;
  }
  ...
  notes.push(
    `wrote video/src/demovideo.content.ts (short: ${short.scenes.length} scenes ${secs(short)}s, ` +
      `long: ${long.scenes.length} scenes ${secs(long)}s @ ${short.fps}fps)`,
  );

  const rootTs = L.readText(path.join(p.video, "src", "Root.tsx")) || "";
  const missing = ["Vertical", "Wide", "WideLong"].filter(
    (id) => !new RegExp(`id="${id}"`).test(rootTs),
  );
  if (missing.length) {
    notes.push(
      `MANUAL STEP: ${missing.join(", ")} not registered in video/src/Root.tsx yet (snippet in SKILL.md, stage 4)`,
    );
  }
  ...
  appendCost(p, {
    stage: "assemble",
    apiUsd: 0,
    shortFrames: short.totalFrames,
    longFrames: long.totalFrames,
  });
