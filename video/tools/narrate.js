...
function alignByMatching(words, script) {
  ...
  const sim = (x, y) => {
    ...
    if (
      x.length >= 3 &&
      y.length >= 3 &&
      (x.startsWith(y) || y.startsWith(x) || x.includes(y) || y.includes(x))
    )
      return 0.7;
    return 0;
  };
  ...
  const first = words[0].s,
    last = words[words.length - 1].e;
  let k = 0;
  while (k < n) {
    if (match[k] >= 0) {
      out[k] = { w: script[k], s: words[match[k]].s, e: words[match[k]].e };
      k++;
      continue;
    }
    let r = k;
    ...
    const span = Math.max(to - from, 0.05),
      cnt = r - k;
    for (let q = 0; q < cnt; q++) {
      out[k + q] = {
        w: script[k + q],
...
function whisperWords(wav) {
  ...
  try {
    ...
    cp.execFileSync("mlx_whisper", wArgs, {
      encoding: "utf8",
      stdio: ["ignore", "pipe", "pipe"],
    });
    const jsonFile = path.join(
      tmp,
      path.basename(wav).replace(/\.wav$/i, "") + ".json",
    );
...
async function main() {
  ...
  if (!process.env.OPENAI_API_KEY) {
    ...
    try {
      key =
        cp
          .execFileSync("zsh", ["-ic", 'printf %s "$OPENAI_API_KEY"'], {
            encoding: "utf8",
            timeout: 15000,
    ...
    if (key) {
      ...
      process.stderr.write("OPENAI_API_KEY loaded from zsh config\n");
    } else {
      console.error(
        "OPENAI_API_KEY is not set (checked env, interactive zsh, ~/.zshrc); TTS needs it.",
      );
      return 1;
    }
  }
  for (const bin of ["mlx_whisper", "ffprobe"]) {
    try {
      cp.execFileSync("/usr/bin/which", [bin], { stdio: "ignore" });
    } catch (_) {
      console.error(
        `${bin} is not installed (brew install ${bin === "ffprobe" ? "ffmpeg" : "mlx-whisper"})`,
      );
      return 1;
    }
  }
  ...
  if (args.includes("--voice")) {
    ...
    if (!flagVoice || !VOICES.includes(flagVoice)) {
      console.error(
        `--voice needs one of: ${VOICES.join(", ")}${flagVoice ? ` (got "${flagVoice}")` : ""}`,
      );
      return 1;
    }
  }
  ...
  for (const scene of sb.scenes) {
    ...
    if (!scene.narration) {
      results.push({ id: scene.id, state: "silent" });
      continue;
    }
    ...
    appendCost(p, {
      ...
      textTokensEst: est.textTokens,
      audioSeconds: +duration.toFixed(2),
      ttsUsdEst: est.usdEst,
  ...
  if (json)
    console.log(
      JSON.stringify(
        { model: MODEL, ttsUsdEstTotal: +spent.toFixed(4), results },
        null,
        2,
      ),
    );
    ...
    if (spent)
      console.log(
        `TTS this run: ~$${spent.toFixed(4)} (estimate, logged to video/costs.jsonl; whisper local, $0)`,
      );
  }
  return 0;
}
