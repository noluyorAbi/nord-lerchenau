...
const FORMATS = {
  '9x16': {
    input: 'vertical.mp4', output: 'social-9x16.mp4', render: 'render:vertical',
    args: (i, o) => ['-i', i, '-c', 'copy', '-movflags', '+faststart', o],
  },
  '1x1': {
    input: 'vertical.mp4', output: 'social-1x1.mp4', render: 'render:vertical',
    args: (i, o) => ['-i', i, '-vf', 'crop=1080:1080:0:420',
      '-c:v', 'libx264', '-crf', '20', '-preset', 'medium', '-pix_fmt', 'yuv420p',
      '-c:a', 'aac', '-b:a', '160k', '-movflags', '+faststart', o],
  },
  '16x9': {
    input: 'wide.mp4', output: 'demo-16x9.mp4', render: 'render:wide',
    args: (i, o) => ['-i', i, '-c', 'copy', '-movflags', '+faststart', o],
  },
  '16x9-long': {
    input: 'wide-long.mp4', output: 'demo-16x9-long.mp4', render: 'render:wide-long',
    args: (i, o) => ['-i', i, '-c', 'copy', '-movflags', '+faststart', o],
  },
  gif: {
    input: 'wide.mp4', output: 'preview.gif', render: 'render:wide',
    ...
    seconds: 15,
    args: (i, o) => ['-t', String(FORMATS.gif.seconds), '-i', i,
      ...
      '-vf', 'fps=10,scale=480:-1:flags=lanczos,split[a][b];[a]palettegen=max_colors=96[p];[b][p]paletteuse=dither=bayer',
      '-loop', '0', o],
  },
};
...
function main() {
  ...
  const p = paths();
  try { cp.execFileSync('/usr/bin/which', ['ffmpeg'], { stdio: 'ignore' }); }
  catch (_) { console.error('ffmpeg is not installed (brew install ffmpeg)'); return 1; }

  const { sb } = loadStoryboard(p);
  const fromFlag = args.includes('--formats') ? args[args.indexOf('--formats') + 1].split(',') : null;
  const formats = (fromFlag || (sb && sb.meta && sb.meta.formats) || ['9x16']).map((f) => f.trim());
  ...
  for (const f of formats) {
    ...
    if (!spec) { problems.push(`unknown format ${f} (supported: ${Object.keys(FORMATS).join(', ')})`); continue; }
    const input = path.join(p.out, spec.input);
    if (!L.exists(input)) { problems.push(`${f}: video/out/${spec.input} missing. Render first: cd video && npm run ${spec.render}`); continue; }
    const out = path.join(p.assets, spec.output);
    cp.execFileSync('ffmpeg', ['-y', '-v', 'error', ...spec.args(input, out)]);
    outputs.push({ format: f, file: out, mb: mb(out), seconds: spec.seconds || null });
  }

  if (outputs.length) appendCost(p, {
    stage: 'finish', apiUsd: 0,
    outputs: outputs.map((o) => ({ format: o.format, file: path.relative(p.root, o.file), mb: o.mb })),
  });

  if (json) { console.log(JSON.stringify({ outputs, problems }, null, 2)); }
  else {
    for (const o of outputs) console.log(
